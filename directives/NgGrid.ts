...
export class NgGrid implements OnInit, DoCheck, OnDestroy {
  ...
  // Event Emitters
  @Output() public onDragStart: EventEmitter<NgGridItem> =
    new EventEmitter<NgGridItem>();
  @Output() public onDrag: EventEmitter<NgGridItem> =
    new EventEmitter<NgGridItem>();
  @Output() public onDragStop: EventEmitter<NgGridItem> =
    new EventEmitter<NgGridItem>();
  @Output() public onResizeStart: EventEmitter<NgGridItem> =
    new EventEmitter<NgGridItem>();
  @Output() public onResize: EventEmitter<NgGridItem> =
    new EventEmitter<NgGridItem>();
  @Output() public onResizeStop: EventEmitter<NgGridItem> =
    new EventEmitter<NgGridItem>();
  @Output() public onItemChange: EventEmitter<Array<NgGridItemEvent>> =
    new EventEmitter<Array<NgGridItemEvent>>();
  ...
  public getItemPosition(itemId: string): NgGridItemPosition {
    return this._items.has(itemId)
      ? this._items.get(itemId).getGridPosition()
      : null;
  }
  ...
  public setMargins(margins: Array<string>): void {
    ...
    this.marginRight =
      margins.length >= 2 ? Math.max(parseInt(margins[1]), 0) : this.marginTop;
    this.marginBottom =
      margins.length >= 3 ? Math.max(parseInt(margins[2]), 0) : this.marginTop;
    this.marginLeft =
      margins.length >= 4
        ? Math.max(parseInt(margins[3]), 0)
        : this.marginRight;
  }
  ...
  public addItem(ngItem: NgGridItem): void {
    ...
    if (!this._preferNew) {
      var newPos = this._fixGridPosition(
        ngItem.getGridPosition(),
        ngItem.getSize()
      );
  ...
  private _updatePositionsAfterMaxChange(): void {
    this._items.forEach((item: NgGridItem) => {
      ...
      if (
        this._hasGridCollision(pos, dims) ||
        !this._isWithinBounds(pos, dims, true)
      ) {
  ...
  private _calculateColWidth(): void {
    if (this._autoResize) {
      if (this._maxCols > 0 || this._visibleCols > 0) {
        ...
        var maxWidth: number =
          this._ngEl.nativeElement.getBoundingClientRect().width;
  ...
  private _calculateRowHeight(): void {
    if (this._autoResize) {
      if (this._maxRows > 0 || this._visibleRows > 0) {
        ...
        var rowHeight: number = Math.max(
          Math.floor(maxHeight / maxRows),
          this.minHeight
        );
    ...
    if (
      ...
      this.minRows > this._config.min_rows
    ) {
      this.minRows = Math.max(
  ...
  private _calculateGridPosition(
    ...
    var col = Math.max(
      1,
      Math.round(left / (this.colWidth + this.marginLeft + this.marginRight)) +
        1
    );
    var row = Math.max(
      1,
      Math.round(top / (this.rowHeight + this.marginTop + this.marginBottom)) +
        1
    );
  ...
  private _fixGridCollisions(
    ...
    for (let collision of collisions) {
      ...
      let newItemPos: NgGridItemPosition = {
        col: itemPos.col,
        row: itemPos.row,
      };
  ...
  private _cascadeGrid(pos?: NgGridItemPosition, dims?: NgGridItemSize): void {
    ...
    let itemsInGrid: NgGridItem[] = Array.from(
      this._itemsInGrid,
      (itemId: string) => this._items.get(itemId)
    );

    switch (this.cascade) {
      case "up":
      case "down":
        itemsInGrid = itemsInGrid.sort(
          NgGridHelper.sortItemsByPositionVertical
        );
        const lowestRowPerColumn: Map<number, number> = new Map<
          number,
          number
        >();
        ...
        for (let item of itemsInGrid) {
          ...
          let lowestRowForItem: number =
            lowestRowPerColumn.get(itemPos.col) || 1;

          for (let i: number = 1; i < itemDims.x; i++) {
            const lowestRowForColumn =
              lowestRowPerColumn.get(itemPos.col + i) || 1;
          ...
          if (pos && dims) {
            const withinColumns =
              rightCol > pos.col && leftCol < pos.col + dims.x;
          ...
          for (let i: number = 0; i < itemDims.x; i++) {
            lowestRowPerColumn.set(
              itemPos.col + i,
              lowestRowForItem + itemDims.y
            ); // Update the lowest row to be below the item
          }
        }
        break;
      case "left":
      case "right":
        itemsInGrid = itemsInGrid.sort(
          NgGridHelper.sortItemsByPositionHorizontal
        );
        const lowestColumnPerRow: Map<number, number> = new Map<
          number,
          number
        >();
        ...
        for (let item of itemsInGrid) {
          ...
          let lowestColumnForItem: number =
            lowestColumnPerRow.get(itemPos.row) || 1;

          for (let i: number = 1; i < itemDims.y; i++) {
            let lowestOffsetColumn: number =
          ...
          for (let i: number = 0; i < itemDims.y; i++) {
            lowestColumnPerRow.set(
              itemPos.row + i,
              lowestColumnForItem + itemDims.x
            ); // Update the lowest col to be below the item
          }
        }
        break;
      default:
  ...
  private _fixGridPosition(
    ...
    if (this._itemFixDirection === "vertical") {
      fixLoop: for (; newPos.col <= maxRow; ) {
        const itemsInPath = this._getItemsInVerticalPath(
          newPos,
          dims,
          newPos.row
        );
      ...
      fixLoop: for (; newPos.row <= maxRow; ) {
        const itemsInPath = this._getItemsInHorizontalPath(
          newPos,
          dims,
          newPos.col
        );
  ...
  private _getMaxRow(): number {
    const itemsRows: number[] = Array.from(
      this._itemsInGrid,
      (itemId: string) => {
        const item = this._items.get(itemId);
        if (!item) return 0;
  ...
  private _getMaxCol(): number {
    const itemsCols: number[] = Array.from(
      this._itemsInGrid,
      (itemId: string) => {
        const item = this._items.get(itemId);
        if (!item) return 0;
  ...
  private _getContainerColumns(): number {
    ...
    const itemWidth: number =
      this.colWidth + this.marginLeft + this.marginRight;
    return Math.floor(maxWidth / itemWidth);
  }

  private _getContainerRows(): number {
    const maxHeight: number =
      window.innerHeight - this.marginTop - this.marginBottom;
    return Math.floor(
      maxHeight / (this.rowHeight + this.marginTop + this.marginBottom)
    );
  }
  ...
  private _getItemFromPosition(position: NgGridRawPosition): NgGridItem {
    return Array.from(this._itemsInGrid, (itemId: string) =>
  ...
  private _createPlaceholder(item: NgGridItem): void {
    ...
    const factory =
      this.componentFactoryResolver.resolveComponentFactory(NgGridPlaceholder);
  ...
  private _enableMouseListeners(): void {
    const documentMousemoveSubs = this._documentMousemove$.subscribe(
      (e: MouseEvent) => this.mouseMoveEventHandler(e)
    );
    const documentMouseupSubs = this._documentMouseup$.subscribe(
      (e: MouseEvent) => this.mouseUpEventHandler(e)
    );
